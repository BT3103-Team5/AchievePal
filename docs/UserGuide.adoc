...
.  The chat room is a place where everyone in the class can share their opinion and discuss about class questions
.  Both students and instructors can use the chat box to interact with anyone else in the same class.

=== Refresh the data 
By click the button on the side bar, latest live data from the Achievement API will be loaded in after seconds and the visualization will change automatically without reloading the page.
